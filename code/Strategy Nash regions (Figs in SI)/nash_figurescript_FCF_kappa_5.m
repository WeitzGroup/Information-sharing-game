...
pvec = .5:.001:1;
qvec = .5:.001:1;
LF_nash = zeros(length(qvec),length(pvec)); % leader-follower
...
for y = 1 : length(qvec)
    ...
    for x = 1 : length(pvec)
        ...
        Q = get_Q(p,q,cA,cB);
        [L0,L1] = get_L(p,q);
        LF_nash(y,x) = is_nash(FC,F,Q,eA,eB,L0,L1,A0,A1);
    end
end
        
LF_nash(LF_nash > 0) = 50; % color scheme for FC-F region
image(pvec,qvec,LF_nash)
axis xy
...
text(.6, .95, 'FC-F','interpreter','latex','fontsize',20)
...
saveas(gcf,'nash_FCF_kappa=5.jpg')
crop('nash_FCF_kappa=5.jpg')
